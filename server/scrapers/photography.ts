...
export const scrapePhotography = async (query: string, location: string): Promise<Lead[]> => {
  ...
  try {
    const searchResult = await withRetry(() =>
      client.search(`${query} ${location} photography`, { limit: 10 })
    );

    if (!searchResult.success) return getMockLeads('photography', query, location);
    ...
    console.error('Photography scrape error (after retries):', error);
